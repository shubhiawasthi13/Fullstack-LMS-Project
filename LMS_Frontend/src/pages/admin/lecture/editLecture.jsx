...
import { useEditLectureMutation, useGetCourseLectureQuery, useRemoveLectureMutation} from "@/features/api/courseApi";
...
function EditLecture() {
  ...
  useEffect(() => {
    if (lectureData?.lectures?.length > 0) {
      const lecture = lectureData.lectures.find((l) => l._id === lectureId);
      if (lecture) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white p-6 transition-colors">
      ...
      {lectureLoading ? (
        ...
        <>
          <Button variant="destructive" className="mb-3 mt-2" onClick={handleRemoveLecture}>
            Remove Lecture
          </Button>
